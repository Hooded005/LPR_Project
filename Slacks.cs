...
namespace Project
{
    internal class Slack
    {
        ...
        public static int findingLimitLower(double decimalVal)
        {
            double lower;

            lower = Math.Floor(decimalVal);

            return (Convert.ToInt32(lower));
        }
        public static List<List<double>> addingSlackConstraint(List<List<double>> table, List<double> rhs, int pivitCol, double dec, int lastCol)
        {
            int lower = findingLimitLower(dec);
            ...
            for (int i = 0; i <= lastCol; i++)
            {
                ...
                else if ((i == (pivitCol)) || (i == (lastCol - 1)))
                {
                    newRow.Add(1);
                }
                else
            ...
            for (int i = 0; i < table.Count; i++)
            {
                if (i == table.Count - 1)
                {
                    table[i].Insert(table[i].Count - 3, 0);
                }
                else
                {
                    table[i].Add(0);
        ...
        public static List<List<double>> simplifyConstraintSlack(List<List<double>> table, int pivitCol, int lastRow, int lastCol)
        {
